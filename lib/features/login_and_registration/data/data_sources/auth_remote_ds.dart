...
class AuthRemoteDS {
  ...
  Future<TokensEntity> getAccessTokenWithEmailAndPassword(
      {required String email, required String password}) async {
    final res = (await AsklessClient.instance
        .create(route: "login", body: {"email": email, "password": password}));
  ...
  Future<void> authenticateWithAccessToken(
      {required String accessToken, bool neverTimeout = false}) async {
    final res = (await AsklessClient.instance.authenticate(
        credential: {"accessToken": accessToken}, neverTimeout: neverTimeout));
  ...
  Future<TokensEntity> useRefreshTokenToGetNewAccessToken(
      {required int userId, required String refreshToken}) async {
