...
class MessagesRemoteDS {
  ...
  Stream<List<int>> streamConversationsWithUnreceivedMessages() {
    ...
    return AsklessClient.instance
        .readStream(
          route: 'conversations-with-unreceived-messages',
        )
        .map((event) => List<int>.from(event));
  }

  Future<DateTime> notifyLoggedUserReadConversation(
      {required int senderUserId, required String lastMessageId}) async {
    final res = await AsklessClient.instance.create(
        route: 'messages-were-read',
        body: {"senderUserId": senderUserId, "lastMessageId": lastMessageId});
    log(
      "notifyLoggedUserReadConversation: ${res.success ? "send successfully" : "failed with code: ${res.error!.code} and description:  ${res.error!.description}"}",
    );
    if (!res.success) {
      throw Failure();
    }
    return DateTime.fromMillisecondsSinceEpoch(
        res.output["readAtMsSinceEpoch"]);
  }

  Future<void> notifyLoggedUserIsTyping(
      {required SendingTypingEntity data}) async {
    final res = await AsklessClient.instance.create(
        route: 'user-typed', body: SendingTypingModel.fromEntity(data).toMap());
    log(
      "notifyLoggedUserIsTyping: ${res.success ? "send successfully" : "failed with code: ${res.error!.code} and description:  ${res.error!.description}"}",
    );
    if (!res.success) {
      throw Failure();
    }
  ...
  Future<MessageEntity> sendMessage(
    ...
    final res = await AsklessClient.instance.create(
        route: 'message',
        body: SendingMessageModel.fromEntity(message).toMap(),
        neverTimeout: true);
    log(
      "sendMessage: ${res.success ? "send successfully" : "failed with code: ${res.error!.code} and description:  ${res.error!.description}"}",
    );
    if (!res.success) {
