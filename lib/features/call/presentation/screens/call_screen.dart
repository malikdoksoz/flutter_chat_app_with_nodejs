...
class _CallScreenState extends State<CallScreen> {
  ...
  static const Duration
      expirationInSecondsToCloseCallWhenRemoteUserIsDisconnected =
      Duration(seconds: 15);
  bool disposed = false;

  final stopwatchController = StopwatchController();
  final reverseStopwatchController = StopwatchController(
      reverseDuration:
          expirationInSecondsToCloseCallWhenRemoteUserIsDisconnected);
                ...
                if (callStatus != CallStatus.callRunning || !args.videoCall)
                  Expanded(
                    child: InfoContent(
                      ...
                      loggedUserIsDisconnected:
                          AsklessClient.instance.connection.status !=
                    ...
                    child: Wrap(
                      ...
                      children: [
                        ...
                        if (callStatus == CallStatus.receivingCall)
                          _BottomButton(
                            ...
                            onTap: () {
                              ...
                              args.receivingCall!
                                  .acceptCall(localStream: localUserStream!)
                                  .then((AcceptingCallResult result) {
                                print(
                                    "call accepted by me: ${result.liveCall != null}");
                                if (result.success) {
                                  ...
                                  handleCallFailed(
                                      error: result.error,
                                      message:
                                          "Ops, sorry, an error occurred when accepting the call, please try again later");
                                }
                              });
                            },
                          ),
                        if (callStatus == CallStatus.callRunning) ...[
                          _BottomButton(
                              iconData: isMicrophoneOn
                                  ? Icons.mic_off_sharp
                                  : Icons.mic_sharp,
                              onTap: toggleAudioOnOff),
                          if (args.videoCall)
                            _BottomButton(
                                iconData: isVideoOn
                                    ? Icons.videocam_off
                                    : Icons.videocam,
                                onTap: toggleVideoOnOff),
                        ]
                      ],
                    ),
                  ),
  ...
  Future<void> _init() async {
    callStatus = args.receivingCall != null
        ? CallStatus.receivingCall
        : CallStatus.requestingCall;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        final localUserStream = await navigator.mediaDevices.getUserMedia({
          'audio': true,
        });
        if (args.videoCall) {
          // https://stackoverflow.com/a/73647944/4508758
          final localVideoStream = await navigator.mediaDevices.getUserMedia({
            'video': true,
          });
          localUserStream.addTrack(localVideoStream.getVideoTracks().last,
              addToNative: true);
        }
        ...
        print(
            "Could not get access to camera and/or microphone ${error.toString()}");
        showSnackBarWarning(
            context: context,
            message:
                'Please, check the ${args.videoCall ? "camera and/or microphone permissions" : "microphone permission"}');
        Navigator.of(context).pop();
      }
    });
  }
  ...
  void handleCallResult(RequestCallResult response) {
    print(
        ">>> handleCallResult: callAccepted -> ${response.callAccepted}: ${response.liveCall != null}");
  ...
  void handleCallStarted(LiveCall liveCall) {
    setState(() {
      ...
      liveCall.addOnRemoteUserConnectionChangeListener(
          closesCallAutomaticallyIfRemoteUserIsDisconnected);
  ...
  void closesCallAutomaticallyIfDisconnected(
      {required bool connected, Duration? customReverseDuration}) {
    setState(() {
      if (disposed) {
      ...
      if (connected) {
        ...
        reverseStopwatchController.start(
            customReverseDuration: customReverseDuration);
...
class InfoContent extends StatelessWidget {
  ...
  const InfoContent(
      {required this.loggedUserIsDisconnected,
      required this.args,
      required this.remoteUserFullName,
      required this.reverseStopwatchController,
      this.disconnectedAt,
      this.message,
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          if (callStatus == CallStatus.callRunning ||
              stopwatchController.seconds > 0) ...[
            Expanded(child: Container()),
            if (disconnectedAt != null) ...[
              Text(
                loggedUserIsDisconnected
                    ? "You are disconnected"
                    : "$remoteUserFullName is disconnected",
                style: TextStyle(
              ...
              StopwatchWidget(
                  ...
                  controller: reverseStopwatchController,
                  fontSize: 16),
              const SizedBox(
                height: 20,
              ),
...
enum CallStatus { requestingCall, receivingCall, callRunning, closed }
