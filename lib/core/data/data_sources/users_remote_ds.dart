...
class UsersRemoteDS {
  ...
  Stream<List<UserModel>> streamUsersToTalk() {
    final stream = AsklessClient.instance
        .readStream(route: 'user-list', source: StreamSource.cacheAndRemote);
    return stream.map((output) => UserModel.fromList(output));
  }

  /// Fetches for the users
  Future<List<UserModel>> readUsersToTalk() async {
    final res = await AsklessClient.instance.read(
      route: 'user-list',
    );
    if (res.success) {
  ...
  Future<UserModel> createUser(
    ...
    final res = await AsklessClient.instance.create(route: 'user', body: {
      "firstName": firstName,
      "lastName": lastName,
      "email": email,
      "password": password,
